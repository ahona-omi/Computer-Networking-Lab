...
ALPHA = 0.125  # Weight constant for EWMA
BETA = 0.25    # Weight constant for calculating Timeout deviation

# Global variables for EWMA calculation
SampleRTT = 0.0
EstimatedRTT = 0.0
DevRTT = 0.0
Timeout = 0.4
...
server_socket.bind(('localhost', 8582))
...
while True:
    ...
    while time.time() - stime < timeout and rwnd > curr_sent:
        ...
        # Start timer for SampleRTT calculation
    ...
    if acknowledgment:
        ...
        if acknowledgment_sequence_number == sequence_number + payload_size:
            ...
            print(
                f'Received acknowledgment for packet {acknowledgment_sequence_number}, but expected {sequence_number}')
    else:
        print('Did not receive acknowledgment')

    # Calculate SampleRTT and update EstimatedRTT and Timeout
    SampleRTT = time.time() - start_time
    EstimatedRTT = (1 - ALPHA) * EstimatedRTT + ALPHA * SampleRTT
    DevRTT = (1 - BETA) * DevRTT + BETA * abs(SampleRTT - EstimatedRTT)
    Timeout = EstimatedRTT + 4 * DevRTT

    # Close connection if all packets are sent and acknowledged
    if not payload:
        break
