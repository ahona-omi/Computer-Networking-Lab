...
client_socket.connect(('localhost', 8582))
...
# Constants for Cumulative ACK and Fast Retransmit
        ...
        while True:
            ...
            while time.time() - start_time < rcvtim:
                ...
                if sequence_number == expected_sequence_number:
                    print(f'ENTERING... {sequence_number} {expected_sequence_number}')
                    data_buffer += payload
                    total_received += len(payload)
                    expected_sequence_number += payload_size  # Update expected sequence number
                    # Construct acknowledgment packet
                    rwnd = int((max_buffer_size - len(data_buffer)) / mss)
                    checksum = 45  # Placeholder for checksum, adjust as needed
                    ack_packet = make_packet(ack, sequence_number + payload_size, rwnd, checksum)
                    ...
                    rwnd = int((max_buffer_size - len(data_buffer)) / mss)
                    checksum = 45  # Placeholder for checksum, adjust as needed
                    ack_packet = make_packet(ack, expected_sequence_number, rwnd, checksum)
                    print('Sending acknowledgment packet...')
                    client_socket.send(ack_packet)  # Send acknowledgment packet to the server
            ...
            if last_ack_number == ack:
                duplicate_ack_count += 1
                if duplicate_ack_count == 3:
                    print("Fast Retransmit: Resending packet...")
                    client_socket.send(packet)
            else:
                duplicate_ack_count = 0
