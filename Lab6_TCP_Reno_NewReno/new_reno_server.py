...
server_socket.bind(('192.168.0.110', 8882))
...
sequence_number = random.randint(0, 2**32)  # Random initial sequence number
ack_number = 0
starting_time = time.time()
file_size = os.path.getsize('sending_file.txt')
last_time = time.time()

# Define packet loss probability (adjust as needed)
...
while True:
    ...
    while max_cap > curr_sent:
        ...
        if random.random() < packet_loss_prob:
            print(f'Packet with seq {sequence_number} lost')
            continue
